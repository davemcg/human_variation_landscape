...
def coding_pos_processor(coding_pos):
	...
	bad_pos = []
	for x in coding_pos:
		if ';' in x: #multiple transcripts for a gene
			try:
			...
			x = int(x) * 3 #multiplying by 3 because dbnSFP gives positions by codon
			int_coding.append(x)
		index = index + 1
...
def central_depot(variant_data, tx_gene_coords, output):
	...
	codon_index = ([x[0] for x in enumerate(header.split('\t')) if x[1] == 'codonpos'])[0]
	CADD_raw_index = ([x[0] for x in enumerate(header.split('\t')) if x[1] == 'CADD_raw'])[0]
	gerp_index = ([x[0] for x in enumerate(header.split('\t')) if x[1] == 'GERP++_NR'])[0]
	...
	for line in dropwhile(lambda line: line.startswith('#'), variant_data):	
		...
		for key, chunk in groupby(variant_data, lambda x: x.split()[Ensembl_geneid_index]):
			chunk = list(chunk)
			...
			try:
				...
				transcripts = list(set([x.split('\t')[tx_index] for x in chunk]))
			for tx in transcripts:
				# grab tx specific positions
				tx_indices = [x[0] for x in enumerate(chunk) if tx in x[1]]
				tx_data = 
